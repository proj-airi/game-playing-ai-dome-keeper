...
- Use an explicit shaft-alignment state before mining and keep mining decisions action-observable rather than reading hidden map shape. Each mining leg moves five tiles horizontally in the current direction. Reaching the target or stalling starts a down-only probe; moving down one tile restarts a five-tile leg in the same direction. If the down probe makes no movement or drilling progress for four seconds, probe one tile horizontally in the opposite direction; success restarts a five-tile leg in that new direction. This is the mirrored state sequence `MineRight -> StuckDown -> MineRight` or, when down fails, `MineRight -> StuckDown -> StuckLeft -> MineLeft`. Save the mining frontier before returning and reuse the target game's bounded open-tile A* path for both returning through mined tunnels and resuming that frontier after unloading or defense; do not retain a second breadcrumb navigation system. When shaft alignment or A* navigation stalls, try one-tile probes in the fixed cycle right, down, left, up, starting after the failed direction, and fail only after all four probes fail. Wait for the pause UI and input-processor transitions to finish before emitting gameplay keys, and fail closed when bounded recovery is exhausted. References: internal project decision, https://docs.godotengine.org/en/4.3/classes/class_astar2d.html#class-astar2d-method-get-point-path, https://docs.godotengine.org/en/4.3/tutorials/inputs/inputevent.html#how-does-it-work
