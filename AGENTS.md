...
- Treat either one map-tile of directed movement or an effective drill hit as progress. After four seconds without either, enter `RECOVER`, try one-tile probes clockwise starting after the failed direction, and return to the interrupted state after the first successful move or effective drill hit. Fail closed after all four directions fail. Reference: internal project decision.
