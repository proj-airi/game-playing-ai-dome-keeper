...
- [PyTorch Transfer Learning Tutorial](https://docs.pytorch.org/tutorials/beginner/transfer_learning_tutorial.html) — The tutorial replaces a pretrained classifier's final layer and fine-tunes the network. It supports joint training of the Lower v0 visual network and action predictor.
- [PyTorch One-Hot Encoding](https://docs.pytorch.org/docs/stable/generated/torch.nn.functional.one_hot.html) — The official operation maps categorical indices to fixed-width one-hot tensors. ADR-0005 uses this representation for action history and structured instructions.
- [PyTorch CrossEntropyLoss](https://docs.pytorch.org/docs/stable/generated/torch.nn.CrossEntropyLoss.html) — The loss accepts class scores and target class indices. It supports the nine-class supervised action objective in ADR-0005.
...
- [Hi Robot](https://www.pi.website/research/hirobot) — Physical Intelligence's hierarchical vision-language-action system, used as evidence for separating slower reasoning over complex instructions and feedback from faster low-level action execution; it is a design reference rather than a selected implementation.
- [RT-H: Action Hierarchies Using Language](https://rt-hierarchy.github.io/) — The primary project page for a hierarchy that predicts language motions between high-level tasks and low-level actions, used to motivate testing language as one possible Upper-to-Lower representation.
- [STEVE-1 project](https://sites.google.com/view/steve-1) — The primary project page for a Minecraft behavior model conditioned by text and visual prompts and producing native keyboard-and-mouse control from pixels, used as a game-domain Lower-Agent reference.
- [STEVE-1 paper](https://proceedings.neurips.cc/paper_files/paper/2023/file/dd03f856fc7f2efeec8b1c796284561d-Paper-Conference.pdf) — The primary description of visual-goal conditioning, packed hindsight relabeling, the text-to-visual CVAE prior, datasets, training configuration, ablations, and reported cost.
- [STEVE-1 source](https://github.com/Shalev-Lifshitz/STEVE-1) — The public policy, dataset-generation, future-goal sampling, CVAE-prior, training, and evaluation implementation used to check details not explicit in the paper.
- [MineDojo and MineCLIP paper](https://arxiv.org/abs/2206.08853) — The primary description of MineCLIP's paired text and 16-frame video embedding space, contrastive training data, architecture, and original training configuration.
- [MineCLIP source](https://github.com/MineDojo/MineCLIP) — The public MineCLIP model and inference implementation, pretrained weights, and video-clip index release; the repository does not include the paper's complete pretraining entry point.
- [Video PreTraining paper](https://cdn.openai.com/vpt/Paper.pdf) — The primary description of VPT's contractor-labeled inverse dynamics model, web-video pseudo-labeling, causal behavioral cloning, focused imitation fine-tuning, and optional reinforcement-learning fine-tuning.
- [Video PreTraining source](https://github.com/openai/Video-Pre-Training) — The public VPT model, weights, contractor samples, IDM demonstration, and consumer-oriented behavioral-cloning example; its README states that the example is not an exact reproduction of paper-scale training.
- [Video PreTraining project article](https://openai.com/index/vpt/) — OpenAI's author overview of the motivation for non-causal action inference, 70,000-hour behavior pretraining, and later behavioral-cloning and reinforcement-learning stages.
- [Optimus-1](https://arxiv.org/abs/2408.03615) — The paper describing a Minecraft agent with a knowledge-guided planner, reflection, and hybrid multimodal memory, used as a long-horizon game-planning and memory reference rather than as a proposed runtime dependency.
- [SayCan](https://say-can.github.io/) — The primary project page for selecting LLM-proposed steps using the grounded affordances of executable low-level skills, used as a reference for constraining Upper-Agent plans to current capabilities.
- [Project Nabla announcement](https://www.reddit.com/r/MachineLearning/comments/wuw2xs/p_imitation_learning_rl_in_super_smash_bros_melee/) — The surviving project announcement describes behavior cloning from tournament replays and light self-play reinforcement learning. The linked technical article is unavailable.
- [Project Slippi replay specification](https://github.com/project-slippi/slippi-wiki/blob/master/SPEC.md) — The replay format records per-frame controller inputs and structured game state for Super Smash Bros. Melee.
- [Phillip source](https://github.com/vladfi1/phillip) — The public implementation of a Melee Agent that uses structured game state and pure reinforcement learning.
- [Phillip paper](https://arxiv.org/abs/1702.06230) — The primary description of Phillip's observation, 54-action space, reward, parallel sampling, and self-play training.
- [At Human Speed](https://arxiv.org/abs/1810.07286) — The Phillip follow-up adds action delay, a delayed-action queue, a recurrent core, and state prediction.
- [Slippi-AI](https://github.com/vladfi1/slippi-ai) — The public Phillip successor combines behavior cloning from Slippi replays with self-play reinforcement learning.
