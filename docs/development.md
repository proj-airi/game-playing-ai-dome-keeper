...
- `LemonNekoGH-DataCollectorAI` is the replacement TypeScript-authored Godot mod. Its source lives in `src/`; `tstogd convert` emits the loadable GDScript into `mods-unpacked/LemonNekoGH-DataCollectorAI/`, alongside the tracked mod manifest. The generated GDScript is ignored (`mods-unpacked/**/*.gd`). Its structure is divided into a planner, task executor, and `Quark Action` executor, replacing the old YOLO collector and rule-teacher organization. The Mod's `tstogd.json` points `gdDir` at `mods-unpacked/LemonNekoGH-DataCollectorAI/`, whose name mirrors the Mod Loader deployment location, so tstogd emits `res://mods-unpacked/...` preload paths that work at runtime.

## Mod Dev Workflow

- Keep mod source in this repo under `mods/<Author>-<ModName>/`. The decompile workflow links only the active TypeScript-authored DataCollectorAI Mod and removes an existing repository-owned legacy YOLO Mod link. DataCollectorAI's loadable root is its generated `mods-unpacked/LemonNekoGH-DataCollectorAI/` directory because that directory contains the runtime GDScript and manifest; the decompile script links it into each decompiled project's `mods-unpacked/`. The decompile, Godot open, and Godot check tasks own the required production build. Mod-specific tests and fixtures live under that Mod's `test/` directory; tests value-import their fixtures, and ViDot compiles them outside the production output root with the nearest `tsconfig.godot.json`. Cross-file references between the Mod's own TypeScript classes use normal imports; the runtime `res://mods-unpacked/...` paths come from the tstogd `gdDir` configuration rather than from string `preload` evals.
