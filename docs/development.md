...
- `LemonNekoGH-DataCollectorAI` is the replacement TypeScript-authored Godot mod. Its source lives in `src/`. `tstogd convert` emits loadable GDScript into `mods-unpacked/LemonNekoGH-DataCollectorAI/`, beside the tracked manifest. The generated GDScript is ignored (`mods-unpacked/**/*.gd`). Its planner, task executor, and `Quark Action` executor replace the old YOLO collector and rule-teacher structure. The Mod's `tstogd.json` sets `lib: true`, so internal imports use relative GDScript paths. Thus, one built package works at the Mod Loader location and through the tstogd test mount.

## Mod Dev Workflow

- Keep mod source in this repo under `mods/<Author>-<ModName>/`. The decompile workflow links only the active TypeScript-authored DataCollectorAI Mod. It removes an existing repository-owned legacy YOLO Mod link. DataCollectorAI's loadable root is its generated `mods-unpacked/LemonNekoGH-DataCollectorAI/` directory. This directory contains the runtime GDScript and manifest. The decompile script links it into the `mods-unpacked/` directory of each decompiled project. The decompile, Godot open, and Godot check tasks own the required production build. Mod-specific tests and fixtures live under the Mod's `test/` directory. The test directory is a separate tstogd library. It builds fixture GDScript into the ignored `test/dist/godot/` directory. It depends on the prebuilt DataCollectorAI and ViKeeper libraries. ViDot mounts it under `tstogd_modules` when a generated test wrapper imports it. Cross-file references use normal TypeScript imports. tstogd derives relative paths within a library and `res://tstogd_modules/...` paths between libraries.
...
- ViKeeper wraps ViDot with Dome Keeper's headless and Movie Maker launch policies. The caller supplies the editable project explicitly; ViKeeper does not scan projects or inspect manifests. DataCollectorAI's MoveTo fixture selects the built-in single-player Engineer and Laser configuration, constructs a bounded underground map through Dome Keeper's `MapData` API, and positions the Keeper at an explicit test coordinate before exposing readiness.
- Decompilation generates local Dome Keeper script declarations under `packages/vikeeper/fixture/_typings/`. That ignored directory is the shared type source for ViKeeper's Godot Fixture and for Mods that use it; do not generate or copy the game declarations into an individual Mod.
- Treat future gameplay fixtures as sparse scenario definitions rather than serialized tile grids. The definition is one object array containing only cells and objects that differ from the fixture default; every unspecified map cell is very-soft dirt. Entries identify their kind and use Godot's `Vector2i` directly for tile positions, with kind-specific data such as `{ type: "ore", ore_type: "iron", position: Vector2i(1, 2) }`. Other entries may mark empty cells, actual gameplay landmarks such as caves, Relics, energy cores, and Gadgets, or already-loose Drops. The Keeper's initial position and task target remain fixture parameters rather than map entries. Do not introduce an ASCII map, tile legend, fill operation, border operation, coordinate tuple serialization, or external configuration parser for this representation.
- Compile declared static cells into `MapData` before the level is built, then create declared Drops after `Level.drops` is initialized through Dome Keeper's normal `network_spawn` path. Retain each returned `Carryable` by its scenario name so Pickup and Drop tests can assert the exact object. An isolated Pickup proof should begin with a declared loose Drop and must not depend on mining it first; a separate mining integration scenario should place ore in the map and verify that gameplay produces the Drop. Scenario setup remains privileged test-fixture behavior, while all behavior after the fixture reports ready must use normal configured game inputs.
- Run `mise run domekeeper:vidot:test` for the automated MoveTo, Pickup,
  type-directed Drop, and Gadget Chamber proofs. Run
  `mise run domekeeper:vidot:record` to write `recordings/move.avi` for human
  visual inspection; recording is not part of the aggregate check.
