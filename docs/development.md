...
- For the Move integration test, launch DataCollectorAI's dedicated test scene. It selects the built-in single-player Engineer and Laser configuration, constructs the smallest required map through Dome Keeper's `MapData` methods before the landing stage can start procedural generation, and instantiates the ordinary game scene without adding test behavior to the production controller. The map has one entry and only the bounded open space needed for one adjacent-tile Move. This direct construction is temporary until the planned shared map-definition DSL owns test maps; do not commit serialized TileMap cell arrays as test definitions.
- Run `mise run godot:check` after changing a Dome Keeper mod. The task starts the target project headlessly with an isolated temporary user-data directory, verifies that directory from inside Godot, requires DataCollectorAI to enter the scene tree, fails if the legacy YOLO Mod is discovered, and ignores known decompiled-project errors outside the active Mod boundary. Build TypeScript-authored mods before this check. The aggregate `mise run check` runs this load check before the Dome Keeper Move test so both processes cannot concurrently write the recovered project's shared `.godot` import cache. Keep the load check in the aggregate task because Godot may exit successfully even after reporting a GDScript parse error.
